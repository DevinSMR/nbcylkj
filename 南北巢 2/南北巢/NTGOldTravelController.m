...
@interface NTGOldTravelController ()<OldTravelTableViewDelegate,NTGOrderTypeDelegate,NTGOrderTypeDelegate,FilterViewDelegate,AttributeViewDelegate>

/** 老年机构列表 */
@property (weak, nonatomic) IBOutlet NTGOldTravelTableView *oldTable;
...
@implementation NTGOldTravelController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"老年旅游";
    ...
    self.oldTable.delegateDock=self;
    [self initData];
...
- (void)initData {
    ...
    // 下拉刷新
    self.oldTable.mj_header= [MJRefreshNormalHeader headerWithRefreshingBlock:^{
    ...
    // 上拉刷新
    self.oldTable.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
...
- (void)updateTableView:(NSArray *)sellers action:(NTGPullRefresh)action state:(BOOL) state {
    if (action == NTGPulRefreshDown || action == NTGPulRefreshInit) {//表示是下拉刷新
        if (state) {
            [self.oldTable clearProducts];
        }
        if (action == NTGPulRefreshDown) {
            [self.oldTable.mj_header endRefreshing];
        }
    }else{//上拉加载
        if (sellers.count == 0) {
            [self.oldTable.mj_footer endRefreshingWithNoMoreData];
        }else{
            [self.oldTable.mj_footer endRefreshing];
        }
    }
    if (state) {
        ...
        [self.oldTable addProducts:sellers];
    }
}
...
//    self.lblTitle.text = _categoryNav.name;
    if ([appPath hasPrefix:@"http://"]) {
        [NTGSendRequest getOldTravel:self.reqParam requestAddr:appPath success:^(NTGProductSellerAttrWrapper *productWrapper) {
            NTGPage *pgModel = productWrapper.page;
//            self.oldTable.products = pgModel.content;
            self.attribute = productWrapper.attribute;
            if (pgModel.content.count == 0) {
                UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 30)];
                self.oldTable.tableFooterView = view;
                UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, view.frame.size.width, view.frame.size.height)];
                label.text = @"抱歉，没有找到符合旅游线路！";
...
//        self.lblTitle.text = @"周边游";
        [NTGSendRequest getOldTravel:self.reqParam requestAddr:@"/app/trip/list.jhtml" success:^(NTGProductSellerAttrWrapper *productWrapper) {
            NTGPage *pgModel = productWrapper.page;
            self.attribute = productWrapper.attribute;
            if (pgModel.content.count == 0) {
                UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 30)];
    ...
    for (int i = 0 ; i< attrs.count; i++) {
        ...
        for (int j = 0 ; j< aAttr.option.count; j++) {
            ...
            if (attibuteOption.tag) {
                if (cell.value.text) {
                    [self.reqParam setObject:attibuteOption.value forKey:aAttr.value];
                }
            }
        }
    }
