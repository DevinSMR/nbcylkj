...
@interface NTGElderlyEstateController ()<ElderlyEstateTableViewDelegate,NTGOrderTypeDelegate,FilterViewDelegate,AttributeViewDelegate>

/** 地产列表 */
@property (weak, nonatomic) IBOutlet NTGElderlyEstateTableView *estateTable;
...
@implementation NTGElderlyEstateController
...
//    self.reqParam = [NSMutableDictionary dictionary];
    ...
    self.estateTable.delegateEld = self;
    [self initData];
...
- (void)initData {
    ...
    // 下拉刷新
    self.estateTable.mj_header= [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        
        [self pageInitData:NTGPulRefreshDown];
    }];
    
    // 上拉刷新
    self.estateTable.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
...
- (void) updateTableView:(NSArray *)sellers action:(NTGPullRefresh)action state:(BOOL) state {
    if (action == NTGPulRefreshDown || action == NTGPulRefreshInit) {//表示是下拉刷新
        if (state) {
            [self.estateTable clearSellers];
        }
        if (action == NTGPulRefreshDown) {
            [self.estateTable.mj_header endRefreshing];
        }
    }else{//上拉加载
        if (sellers.count == 0) {
            [self.estateTable.mj_footer endRefreshingWithNoMoreData];
        }else{
            [self.estateTable.mj_footer endRefreshing];
        }
    }
    if (state) {
        ...
        [self.estateTable addSellers:sellers];
    }
}
...
- (void)initData:(NTGPullRefresh)action {
    ...
    [self.reqParam setValue:@"18" forKey:@"id"];
    if ([appPath hasPrefix:@"http://"]) {
        [NTGSendRequest getElderlyEstate:self.reqParam requestAddr:appPath success:^(NTGProductSellerAttrWrapper *productWrapper) {
            NTGPage *pgModel = productWrapper.page;
            self.attribute = productWrapper.attribute;
            //            self.estateTable.sellers = pgModel.content;
            if (pgModel.content.count == 0) {
                UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 30)];
                self.estateTable.tableFooterView = view;
...
//        self.lblTitle.text = @"养老地产";
        [NTGSendRequest getElderlyEstate:self.reqParam requestAddr:@"app/elderlyEstate/list.jhtml" success:^(NTGProductSellerAttrWrapper *productWrapper) {
            NTGPage *pgModel = productWrapper.page;
            self.attribute = productWrapper.attribute;
//            self.estateTable.sellers = pgModel.content;
            if (pgModel.content.count == 0) {
...
-(void)clickindexPathRow:(NTGElderlyEstateContent *)project index:(int)index {
    UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"NTGEstateDetail" bundle:nil];
    NTGEstateDetailController *estateDetailController = [storyboard instantiateInitialViewController];
    estateDetailController.project = project;
    [self.navigationController pushViewController:estateDetailController animated:YES];
}
...
- (void) initDefaultFilter {
    ...
    defalutAttrOption.value = @"topDesc";
