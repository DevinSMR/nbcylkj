...
@interface NTGProductController ()<ProductTableViewDelegate,NTGOrderTypeDelegate,FilterViewDelegate,AttributeViewDelegate>
...
@implementation NTGProductController
...
//    self.coverFilterView = recognizer.view;
    for (int i = (int)self.view.subviews.count-1; i>=0; i--) {
        UIView *subView =  self.view.subviews[i];
        if ([subView isKindOfClass:[NTGOrderType class]]) {
            [subView removeFromSuperview];
            if (recognizer.view == self.coverView) {
                self.coverView.hidden = YES;
            }
        }
        if ([subView isKindOfClass:[NTGFilterView class]]) {
            [subView removeFromSuperview];
            if (recognizer.view == self.coverFilterView) {
...
- (void)initData {
    ...
    // 下拉刷新
    self.productTable.mj_header= [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        
        [self pageInitData:NTGPulRefreshDown];
    }];
    
    // 上拉刷新
    self.productTable.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
...
- (void) updateTableView:(NSArray *)sellers action:(NTGPullRefresh)action state:(BOOL) state {
    if (action == NTGPulRefreshDown || action == NTGPulRefreshInit) {//表示是下拉刷新
        if (state) {
            [self.productTable clearSellers];
        }
        
        if (action == NTGPulRefreshDown) {
            [self.productTable.mj_header endRefreshing];
        }
    }else{//上拉加载
        if (sellers.count == 0) {
            [self.productTable.mj_footer endRefreshingWithNoMoreData];
        }else{
            [self.productTable.mj_footer endRefreshing];
        }
    }
    if (state) {
        ...
        [self.productTable addSellers:sellers];
    }
}
...
- (void)initData:(NTGPullRefresh)action {
    ...
    if (_categoryNav != nil) {
        appPath = _categoryNav.appPath;
    }else {
        
        NSString *productCategoryId = [_reqParam valueForKey:@"productCategoryId"];
        appPath = [NSString stringWithFormat:@"app/product/list/%@.jhtml",productCategoryId];
    }
    [NTGSendRequest getOldMall:self.reqParam requestAddr:appPath success:^(NTGProductSellerAttrWrapper *productWrapper) {
        ...
        if (pgModel.content.count == 0) {
            if (!self.footView) {
                ...
                self.productTable.tableFooterView = self.footView;
...
//        [self.view addSubview:self.coverFilterView];
...
//    if ([self.reqParam valueForKey:@"_categoryNav_"]) {
//        [self.reqParam removeObjectForKey:@"_categoryNav_"];
//    }

    for (int i = 0 ; i< attrs.count; i++) {
        NTGAttribute *aAttr = (NTGAttribute *)attrs[i];
        NSArray *visibleCells = self.filterView.termListTable.visibleCells;
        for (int k=0; k<visibleCells.count; k++) {
            NTGFilterCell *cell = visibleCells[k];
//        }
...
- (void)clickAttributeViewIndexPathRow:(NTGAttibuteOption *)attibuteOption index:(NSIndexPath *)index {
    ...
    for (int i=0;i<visibleCells.count;i++) {
        ...
        if (otherCell != attributeCell) {
            ...
            attributeCell.name.textColor = [UIColor colorWithRed:248.0/255 green:111.0/255 blue:78.0/255 alpha:1.0];
        }
    }
